...
library(mixSPE)
library(SenTinMixt) # Available at https://github.com/StatLearn123/Parsimonious-Mixtures-of-Dimension-Wise-Scaled-Normal-Mixtures/tree/main
library(pgmm)
library(tcltk)
...
nThreads <- 28 # for parallel computing of DSEN_M.fit() and Mixt.fit()
ncores <- 10 # for parallel computing of Mixt.fit.init()
...
for (l in 1:nsims) {
  ...
  set.seed(123)
  resSEN.init <- tryCatch(Mixt.fit.init(X=X[[l]], k=2, density = "MSEN", ncores = ncores, verbose = TRUE, nstartR = 50), error = function(e) {NA})
  resSEN[[l]] <- tryCatch(Mixt.fit(X=X[[l]], k = 2, init.par = resSEN.init, density = "MSEN", ncores = nThreads, verbose = T), error = function(e) {NA})
  
  #### Multivariate TIN Mixtures (Eigen) ####
  
  set.seed(123)
  resTIN.init <- tryCatch(Mixt.fit.init(X=X[[l]], k=2, density = "MTIN", ncores = ncores, verbose = TRUE, nstartR = 50), error = function(e) {NA})
  resTIN[[l]] <- tryCatch(Mixt.fit(X=X[[l]], k = 2, init.par = resTIN.init, density = "MTIN", ncores = nThreads, verbose = T), error = function(e) {NA})
